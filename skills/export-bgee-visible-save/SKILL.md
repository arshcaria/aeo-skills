...
description: >-
  ...
  after game-rule and equipped-item modifiers. Use for requests to analyze the
...
3. Run `scripts/export_visible_save.mjs` with absolute paths. The script detects SoD from `current_campaign=SOD` and applies resource precedence `override > sod-dlc > base`.
